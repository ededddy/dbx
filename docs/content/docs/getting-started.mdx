...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Docker">
    ...
    Without `DBX_PASSWORD`, the first visit shows a setup page that creates the initial username and password — that first account is the admin. Admin accounts can create and manage additional accounts under **Settings → Security**.
