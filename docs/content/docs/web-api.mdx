...
Setup creates the first user account and is only accepted while no account exists.
...
Any authenticated session can manage user accounts. Accounts created from the `DBX_PASSWORD`/`DBX_USERNAME` environment variables are listed with `managedByEnv: true` and cannot be modified through the API.
...
```json
[
  { "id": 1, "username": "admin", "createdAt": 1754800000000, "managedByEnv": false }
]
```
...
Deleting your own account or the last remaining account is rejected.
