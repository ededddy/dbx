...
Any signed-in account can change its own password by proving the old one. On success, all other sessions of that account are revoked; the session that made the change stays signed in.
...
| `DBX_SESSION_IDLE_TIMEOUT_MINUTES` | Expire sessions idle longer than this many minutes; unset or `0` keeps sessions until logout or restart |
