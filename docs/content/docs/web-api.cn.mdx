...
同一账号连续 5 次登录失败会锁定该账号约 60 秒，不影响其他账号。会话保存在当前 Web 进程内，进程重启后需要重新登录；部署者还可以通过 `DBX_COOKIE_SECURE=1` 为 Cookie 加上 `Secure` 属性，并用 `DBX_SESSION_IDLE_TIMEOUT_MINUTES` 设置空闲会话过期时间。Cookie 使用 `HttpOnly` 和 `SameSite=Lax`，路径会跟随 `DBX_PUBLIC_BASE_PATH`。
...
任何已登录账号都可以凭旧密码修改自己的密码。修改成功后，该账号的其他会话全部失效，发起修改的会话保持登录。
...
只有管理员账号可以管理用户：首次设置创建的账号即为管理员，通过 `DBX_PASSWORD`/`DBX_USERNAME` 环境变量配置的账号始终视为管理员。管理员可以在创建账号时授予管理员权限（`is_admin: true`）。环境变量账号会以 `managedByEnv: true` 列出，无法通过 API 修改。修改密码会使该账号的其他会话失效；重置其他账号的密码会使其全部会话失效。
...
| `DBX_SESSION_IDLE_TIMEOUT_MINUTES` | 空闲超过该分钟数后会话过期；不设置或为 `0` 时会话保持到退出登录或进程重启 |
