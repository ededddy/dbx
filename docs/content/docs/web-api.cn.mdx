...
只有管理员账号可以管理用户：首次设置创建的账号即为管理员，通过 `DBX_PASSWORD`/`DBX_USERNAME` 环境变量配置的账号始终视为管理员。环境变量账号会以 `managedByEnv: true` 列出，无法通过 API 修改。修改密码会使该账号的其他会话失效；重置其他账号的密码会使其全部会话失效。
